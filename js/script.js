...
// Session cart key (auto-clear on page leave)
var SESSION_CART_KEY = 'afrishop_session_cart';

// Merge all products
var products = [].concat(
    (typeof productsElectronics !== 'undefined' ? productsElectronics : []),
    (typeof productsFashion !== 'undefined' ? productsFashion : []),
    (typeof productsHome !== 'undefined' ? productsHome : []),
    (typeof productsBeauty !== 'undefined' ? productsBeauty : []),
    (typeof productsSports !== 'undefined' ? productsSports : []),
    (typeof productsToys !== 'undefined' ? productsToys : [])
);
...
function loadCategories() {
    ...
    categories.forEach(function(cat) {
        ...
        card.innerHTML = '<img src="category/' + cat.image + '" alt="' + cat.name + '"><div class="category-info"><h3>' + cat.name + '</h3><a href="category.html?cat=' + cat.id + '" class="view-more">View More</a></div>';
...
function loadFeaturedProducts() {
    ...
    var featured = shuffled.slice(0, 80);
    featured.forEach(function(product) {
        var discount = calculateDiscount(product.price, product.originalPrice);
        var card = document.createElement('div');
        card.className = 'product-card';
        card.dataset.id = product.id;
        card.innerHTML = '<div class="product-image"><img src="images/' + product.images[0] + '" alt="' + product.name + '">' + (discount > 0 ? '<span class="discount-badge">' + discount + '% OFF</span>' : '') + '</div><div class="product-info"><h3 class="product-name">' + product.name + '</h3><div class="product-price"><span class="current-price">UGX ' + product.price.toLocaleString() + '</span>' + (product.originalPrice > product.price ? '<span class="original-price">UGX ' + product.originalPrice.toLocaleString() + '</span>' : '') + '</div><p class="product-description">' + product.description.substring(0, 50) + '...</p><button class="add-cart-btn">Add to Cart</button></div>';
        grid.appendChild(card);
...
function setupFAB() {
    ...
    if (fabWhatsapp) fabWhatsapp.addEventListener('click', function() { window.open('https://wa.me/256705934559', '_blank'); });
    if (fabCall) fabCall.addEventListener('click', function() { window.location.href = 'tel:+256705934559'; });
...
function loadCartItems() {
    ...
    var cart = getCart();
    if (cart.length === 0) {
        container.innerHTML = '<div class="empty-cart"><p>Your cart is empty</p><a href="index.html" class="continue-shopping">Continue Shopping</a></div>';
        updateCartSummary();
        return;
    }
    container.innerHTML = '';
...
function proceedToCheckout() {
    ...
    var message = 'Hello Afrishop! I would like to order:\n\n';
    ...
    setTimeout(function() { window.open('https://wa.me/256705934559?text=' + encodeURIComponent(message), '_blank'); }, 1000);
...
function setupProductGallery() {
    ...
    if (gallery) {
        gallery.innerHTML = '<div class="main-image"><img src="images/' + product.images[0] + '" alt="' + product.name + '"></div><div class="thumbnail-container"></div>';
    ...
    var addBtn = document.querySelector('.product-details .add-cart-btn');
...
function loadCategoryProducts() {
    ...
    var categoryMap = { electronics: 'Electronics', fashion: 'Fashion', home: 'Home & Kitchen', beauty: 'Beauty & Health', sports: 'Sports', toys: 'Toys' };
